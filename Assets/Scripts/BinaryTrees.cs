...
class BinaryTrees {
	...
	public static void Main( String[] args ) {
		...
		Debug.Log(string.Format("stretch tree of depth {0}\t check: {1}", stretchDepth, check));
		...
		for ( int depth = minDepth; depth <= maxDepth; depth += 2 ) {
			...
			for ( int i = 1; i <= iterations; i++ ) {
				...
				Debug.Log(string.Format("{0}\t trees of depth {1}\t check: {2}",
				iterations * 2, depth, check));
		}
					Debug.Log(string.Format("long lived tree of depth {0}\t check: {1}",
